...
.PHONY: test-e2e
test-e2e: check-go
	go build -race -o artifacts/chro .
	env EDGE_BINARY=${PWD}/artifacts/chro go test -v -timeout=30m ./e2e/...

.PHONY: test-e2e-polybft
test-e2e-polybft: check-go
	go build -o artifacts/chro .
	env EDGE_BINARY=${PWD}/artifacts/chro E2E_TESTS=true E2E_LOGS=true \
	go test -v -timeout=1h30m ./e2e-polybft/e2e/...

.PHONY: test-property-polybft
test-property-polybft: check-go
	go build -o artifacts/chro .
	env EDGE_BINARY=${PWD}/artifacts/chro E2E_TESTS=true E2E_LOGS=true go test -v -timeout=30m ./e2e-polybft/property/... \
	-rapid.checks=10
